...
export async function GET(request: Request) {
  ...
  // DD5 PFU Tremie = mixId HD5KMDD1 only. Without this filter, bulk download
  // would pull every Summary-sheet row (765+ non-Tremie mixes also live here).
  // DD5 PFU Tremie started Nov 10 2025 — filter in SQL, not JS.
  const conditions = [
    ...
    if (r.ticketFileUrl) {
      try {
        ...
        if (res.ok) {
          ...
          finalPdf = await mergeReportWithTicket(buf, ticketBytes)
        }
      } catch { /* skip if ticket fetch fails */ }
    }

    const dateStr = r.shiftDate.replace(/-/g, '')
    const locStr = (r.locationDescription ?? 'Report').replace(/[^a-zA-Z0-9 ]/g, '').trim().replace(/\s+/g, '_')
    files.push({ name: `${dateStr}_${locStr}_Report.pdf`, data: finalPdf })
  }
  ...
  return new NextResponse(Buffer.from(zipBytes), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="compression-reports-dd5-${label}-${Date.now()}.zip"`,
    },
  })
}
